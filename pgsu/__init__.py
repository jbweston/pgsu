...
class PGSUConnectionMode(IntEnum):
...
class PGSU(object):  # pylint: disable=useless-object-inheritance
    ...
    def __init__(self,
        ...
        self.quiet = quiet
        self.connection_mode = PGSUConnectionMode.DISCONNECTED
    ...
    def execute(self, command, **kwargs):
        ...
        if self.connection_mode == PGSUConnectionMode.PSYCOPG:  # pylint: disable=no-else-return
            return _execute_psyco(command, **kw_copy)
        elif self.connection_mode == PGSUConnectionMode.PSQL:
    ...
    def determine_setup(self):
        ...
        for pg_user in set([dbinfo.get('user'), None]):
            ...
            if _try_connect_psycopg(**dbinfo):
                self.dbinfo = dbinfo
                self.connection_mode = PGSUConnectionMode.PSYCOPG
                return True
        ...
        if _sudo_exists():
            if _try_subcmd(interactive=self.interactive,
                           ...
                           **dbinfo):
                self.dbinfo = dbinfo
                self.connection_mode = PGSUConnectionMode.PSQL
                return True
        elif not self.quiet:
            click.echo(
    ...
    def is_connected(self):
        return self.connection_mode in (PGSUConnectionMode.PSYCOPG,
                                        PGSUConnectionMode.PSQL)
